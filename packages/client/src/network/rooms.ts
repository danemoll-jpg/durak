...
// each other and, once a game starts, to sync turns. Same pattern as the Par Five
// project's src/network/rooms.js, adapted for Durak's hidden-hand game state.
//
...
//     commentary: [{ seq, speakerId, personality, text }],  // rolling window, newest
//       last, capped to COMMENTARY_LIMIT. Only ever appended by whoever just wrote a new
...
//       so they don't need to live here at all.
//     nextCommentarySeq: <number>,  // monotonic, so every commentary entry gets a unique seq
//   }
// A seat with clientId === null and type "human" is an open seat — whoever calls
// joinRoom() next claims the first one they find.

import { doc, onSnapshot, runTransaction, setDoc, updateDoc } from 'firebase/firestore';
import { BotPersonalityId, CommentaryLine, createGame, GameState } from '@durak/engine';
...
/** Host action: deals the game and moves the room into "playing". */
export async function startGame(code: string, seats: RoomSeat[]): Promise<void> {
 ...
 * new commentary array/seq so a caller stepping several moves in a row (the host driving
 * a run of bot turns) can keep its own local copy accurate between snapshot round-trips,
...
/** Resets a room back to its lobby, keeping the same seats — "New Game" without leaving
 * the room. */
