...
import { SfxCue } from '@durak/engine';

export type SoundName = Exclude<SfxCue, 'gameOver'> | 'gameOverWin' | 'gameOverLose' | 'gameOverDraw';

const MUTE_KEY = 'durak-muted';
...
const PLAYERS: Record<SoundName, () => void> = {
  ...
  cardThrow: () => tone(620, 0, 0.07, 'square', 0.1),
  cardDefend: () => {
    tone(520, 0, 0.05, 'square', 0.11);
    tone(780, 0.05, 0.09, 'square', 0.11);
  },
  take: () => {
    sweep(320, 90, 0, 0.32, 'sawtooth', 0.18);
    tone(90, 0.28, 0.14, 'square', 0.13);
  },
  deckEmpty: () => tone(180, 0, 0.35, 'sawtooth', 0.13),
  safe: () => {
    tone(660, 0, 0.08, 'triangle', 0.14);
    tone(880, 0.08, 0.12, 'triangle', 0.14);
  },
  gameOverWin: () => {
